...
static char const * colorChooserWinConsole(
	char const * const aTitle, /* NULL or "" */
	char const * const aDefaultHexRGB, /* NULL or "#FF0000"*/
	unsigned char aDefaultRGB[3], /* { 0 , 255 , 255 } */
	unsigned char aoResultRGB[3]) /* { 0 , 0 , 0 } */
...
{
	return !( tinyfd_forceConsole && isatty ( 1 ) ) && getenv ( "DISPLAY" );
}
    ...
    int const aDefaultButton ) /* 0 for cancel/no , 1 for ok/yes */
{
	...
	if ( osascriptPresent ( ) && !preferPython() )
    {
    ...
    else if ( zenityPresent() && !preferPython() )
    {
...
if res==False :\n\tprint 0\n\
		   ...
		   || dialogPresent() || whiptailPresent() )
	{
		...
		strcat(lDialogString, "\" 10 60 >/dev/tty) 2>&1;\
if [ $? = 0 ];then echo 1;else echo 0;fi ; clear >/dev/tty");
	}
	else 
	{
		printf ("\n%s\n", gTitle);
...
frontmost of process \\\"Python\\\" to true' ''');");
		   ...
		   || dialogPresent() || whiptailPresent() )
	{
		...
		strcat(lDialogString,
">/dev/tty ) 2>/tmp/tinyfd.txt;if [ $? = 0 ];then tinyfdBool=1;else tinyfdBool=0;fi;\
tinyfdRes=$(cat /tmp/tinyfd.txt);rm /tmp/tinyfd.txt;echo $tinyfdBool$tinyfdRes;clear >/dev/tty") ;
	}
	else 
	{
		printf ("\n%s\n", gTitle);
...
((item i of mycolor)/256 as integer) as string' " );
    ...
    else if ( zenity3Present()  && !preferPython() )
	{
		lWasZenity3 = 1 ;
        sprintf ( lDialogString ,
...
\nif res[1] is not None:\n\tprint res[1]\"" ) ;
	...
	else if ( xdialogPresent() )
	{
		...
		sprintf(lTmp,"%hhu %hhu %hhu",aDefaultRGB[0],aDefaultRGB[1],aDefaultRGB[2]);
		strcat(lDialogString, lTmp) ;
